...
import {
  Option,
} from "@/components/ui/tag-input/UserMultipleSelector";
...
import TreatmentMultipleSelector from "./TreatmentMultipleSelector";
...
export const TreatmentTagInput = forwardRef(
  (
    {
      name,
      formLabel,
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem className="flex flex-1">
            <FormControl className="relative flex flex-col space-y-1 w-full">
              <div>
                <Label
                  htmlFor={name}
                  className={cn(
                ...
                <TreatmentMultipleSelector
                  onSearch={async (value) => {
                    setIsTriggered(true);
                    const res = await fetchTags(
                      { search: value, type: TagType.TREATMENT },
                      {},
                    );
