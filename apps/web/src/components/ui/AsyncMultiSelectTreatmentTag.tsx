...
import { useDynamicGetList, useUpdateTreatmentTag } from "@ttm/api";
...
export const AsyncMultiSelectTreatmentTag = forwardRef(
  (
    {
      name,
      formLabel,
          ...
          .then((values) => {
            if (values && values.length > 0) {
              let formattedValues = values.map((value) => ({
                ...
                treatment_tag_id:
                  dataValue.find((item) => item.id === value["id"])
                    ?.treatment_tag_id || null,
                type:
                  dataValue.find((item) => item.id === value["id"])?.type || "",
              }));
    ...
    return (
      <FormField
        ...
        name={name}
        render={({ field }) => (
          <FormItem className="border-3 relative flex flex-1 flex-col justify-center space-y-1 border-red-500">
            <Label
