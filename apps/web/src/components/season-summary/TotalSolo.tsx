...
export function TotalSolo({ season }: { season: Season | undefined }) {
  const soloData = season?.report_data.total_solo_by_users;

  if (!soloData || soloData.length === 0) {
    return (
      <Card>
        <CardHeader className="pb-0">
          <CardTitle>Solo Task</CardTitle>
        </CardHeader>
