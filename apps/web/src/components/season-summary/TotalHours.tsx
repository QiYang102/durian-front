...
export function TotalHours({ season }: { season: Season | undefined }) {
  const hourData = season?.report_data.total_hours_committed_by_user;

  if (!hourData || hourData.length === 0) {
    return (
      <Card>
        <CardHeader className="pb-0">
          <CardTitle>Hour Committed</CardTitle>
        </CardHeader>
